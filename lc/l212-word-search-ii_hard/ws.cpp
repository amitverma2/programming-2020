    ...
    vector<string> findWords(vector<vector<char>>& board, vector<string>& words) {
      ...
      for (int r = 0; r < board.size(); ++r) {
        for (int c = 0; c < board[r].size(); ++c) {
          ...
          if (cur) {
            findWordsTrie(board, r, c, cur, out);
          }
        }
      }
      return out;
    }
    void findWordsTrie(vector<vector<char>>& board, int r, int c, TrieNode* cur, vector<string>& out) {
      ...
      for (int i = 0 ; i < 4; ++i) {
        ...
        if (r_next >= 0 && r_next < board.size() && c_next >= 0 && c_next < board[r_next].size()) {
          ...
          if (next_char != '#') {
            ...
            if (next_node) {
              findWordsTrie(board, r_next, c_next, next_node, out);
            }
          }
        }
      }
